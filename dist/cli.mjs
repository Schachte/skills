...
async function removeCommand(skillNames, options) {
	...
	const isGlobal = options.global ?? false;
	const cwd = process.cwd();
	const spinner$3 = options.embedded ? null : spinner();
	spinner$3?.start("Scanning for installed skills…");
	...
	spinner$3?.stop(`Found ${installedSkills.length} unique installed skill(s)`);
	...
	if (installedSkills.length === 0 && resolvedRequestedSkills.length === 0) {
		if (options.embedded && skillNames.length > 0) throw new Error(`No matching skills found for: ${skillNames.join(", ")}`);
		if (!options.embedded) outro(import_picocolors.default.yellow("No skills found to remove."));
		return;
	}
	...
	if (options.all) selectedSkills = resolvedRequestedSkills;
		...
		if (selectedSkills.length === 0) {
			if (options.embedded) throw new Error(`No matching skills found for: ${skillNames.join(", ")}`);
	...
	if (options.agent && options.agent.length > 0) targetAgents = options.agent;
		...
		spinner$3?.stop(`Targeting ${targetAgents.length} potential agent(s)`);
	}
	if (!options.yes) {
		console.log();
	...
	spinner$3?.start("Removing skills…");
	const results = [];
	...
	spinner$3?.stop("Removal process complete");
	const successful = results.filter((r) => r.success);
	const failed = results.filter((r) => !r.success);
	if (options.embedded && failed.length > 0) throw new Error(`Failed to remove: ${failed.map((r) => `${r.skill}: ${r.error}`).join(", ")}`);
	...
	if (!options.embedded && successful.length > 0) log.success(import_picocolors.default.green(`Successfully removed ${successful.length} skill(s)`));
...
async function resolveInstallScope(options) {
...
async function enabledNamesForSource(global, source) {
	return getEnabledNamesForSource((global ? await readSkillLock() : await readLocalLock()).skills, source);
}
function getEnabledNamesForSource(entries, source) {
	return new Set(Object.entries(entries).filter(([, entry]) => {
		...
		if (!identity && entry.sourceType === "github") identity = `github:${entry.source.toLowerCase()}`;
...
function getWellKnownOwnershipSource(url) {
	const parsed = new URL(url);
	parsed.hash = "";
	if (parsed.pathname !== "/") parsed.pathname = parsed.pathname.replace(/\/+$/, "");
	return parsed.toString();
}
function getRepositoryOwnershipSource(parsedUrl, normalizedSource) {
	if ((/^git@github\.com:/i.test(parsedUrl) || isGitHubSshSource(parsedUrl) || (() => {
		try {
			return new URL(parsedUrl).hostname.toLowerCase() === "github.com";
		} catch {
			return false;
		}
	})()) && normalizedSource) return `github:${normalizedSource.toLowerCase()}`;
	return getLockSource(parsedUrl, normalizedSource) ?? parsedUrl;
}
function isEnabled(name, enabledNames) {
	return enabledNames.has(sanitizeName(name));
}
function getDeselectedEnabledSkillNames(enabledSkills, selectedSkills, getName) {
	const selectedNames = new Set(selectedSkills.map((skill) => sanitizeName(getName(skill))));
	return enabledSkills.filter((skill) => !selectedNames.has(sanitizeName(getName(skill)))).map(getName);
}
async function applySkillRemovals(skillNames, global) {
	if (skillNames.length === 0) return;
	await removeCommand(skillNames, {
		global,
		yes: true,
		embedded: true
	});
...
async function handleWellKnownSkills(source, url, options, spinner) {
	...
	if (options.skill?.includes("*")) {
		...
		const selected = await searchMultiselect({
			...
			items: skillChoices,
			initialSelected: isSkillsShPackUrl(url) ? Array.from(/* @__PURE__ */ new Set([...skills, ...enabledSkills])) : enabledSkills,
			required: false,
			maxVisible: 20,
			selectAll: true
		});
	...
	if (selectedSkills.length === 0) {
		...
		note(skillsToDisable.map((name) => import_picocolors.default.red(`Disable ${name}`)).join("\n"), "Changes Summary");
	...
	if (installGlobally) {
		const unsupportedAgents = targetAgents.filter((agent) => agents[agent].globalSkillsDir === void 0);
		if (unsupportedAgents.length > 0) {
			log.warn(`Skipping agents without global skill support: ${unsupportedAgents.map((agent) => agents[agent].displayName).join(", ")}`);
			targetAgents = targetAgents.filter((agent) => agents[agent].globalSkillsDir !== void 0);
		}
		if (targetAgents.length === 0) {
			log.error("None of the selected agents support global skill installation.");
			process.exit(1);
		}
	}
	...
	if (skillsToDisable.length > 0) {
		if (summaryLines.length > 0) summaryLines.push("");
		summaryLines.push(import_picocolors.default.bold("Disable"));
		for (const name of skillsToDisable) summaryLines.push(`  ${import_picocolors.default.red(name)}`);
	}
	console.log();
	note(summaryLines.join("\n"), skillsToDisable.length > 0 ? "Changes Summary" : "Installation Summary");
	if (!options.yes) {
		const confirmed = await confirm({ message: skillsToDisable.length > 0 ? "Apply changes?" : "Proceed with installation?" });
		if (isCancel(confirmed) || !confirmed) {
			cancel("Installation cancelled");
			process.exit(0);
		}
	...
	const successful = results.filter((r) => r.success);
	const failed = results.filter((r) => !r.success);
	if (failed.length === 0) await applySkillRemovals(skillsToDisable, installGlobally);
	else if (skillsToDisable.length > 0) log.warn("Skipped disabling skills because installation did not complete successfully.");
	const skillFiles = {};
...
async function runAdd(args, options = {}) {
	...
	try {
		...
		spinner$2.start("Parsing source…");
		const parsed = parseSource(source);
		let directDownload = parsed.type === "download";
		spinner$2.stop(`Source: ${parsed.type === "local" ? parsed.localPath : parsed.url}${parsed.ref ? ` @ ${import_picocolors.default.yellow(parsed.ref)}` : ""}${parsed.subpath ? ` (${parsed.subpath})` : ""}${parsed.skillFilter ? ` ${import_picocolors.default.dim("@")}${import_picocolors.default.cyan(parsed.skillFilter)}` : ""}`);
		...
		if (parsed.type === "well-known") {
			if (await handleWellKnownSkills(source, parsed.url, options, spinner$2)) return;
			directDownload = true;
		}
		...
		if (parsed.type === "local") {
			spinner$2.start("Validating local path…");
			if (!existsSync(parsed.localPath)) {
				spinner$2.stop(import_picocolors.default.red("Path not found"));
				outro(import_picocolors.default.red(`Local path does not exist: ${parsed.localPath}`));
				process.exit(1);
			}
			spinner$2.stop("Local path validated");
			spinner$2.start("Discovering skills…");
			skills = await discoverSkills(parsed.localPath, parsed.subpath, {
				includeInternal,
				fullDepth: options.fullDepth
			});
		} else if (parsed.type === "well-known" || parsed.type === "download") {
			spinner$2.start("Downloading source...");
			const downloaded = await downloadSource(parsed.url);
			tempDir = downloaded.tempDir;
			spinner$2.stop(`Downloaded ${downloaded.kind === "skill-md" ? "SKILL.md file" : "archive"}`);
			spinner$2.start("Discovering skills...");
			...
			if (ownerRepo && owner && (isSelfHostedRepo || BLOB_ALLOWED_OWNERS.includes(owner))) {
				spinner$2.start("Fetching skills…");
				...
				if (!blobResult) spinner$2.stop(import_picocolors.default.dim("Falling back to clone…"));
			}
			if (blobResult) {
				skills = blobResult.skills;
				spinner$2.stop(`Found ${import_picocolors.default.green(skills.length)} skill${skills.length > 1 ? "s" : ""}`);
			} else {
				spinner$2.start("Cloning repository…");
				tempDir = await cloneRepo(parsed.url, parsed.ref);
				spinner$2.stop("Repository cloned");
				spinner$2.start("Discovering skills…");
		...
		if (skills.length === 0) {
			spinner$2.stop(import_picocolors.default.red("No skills found"));
			outro(import_picocolors.default.red("No valid skills found. Skills require a SKILL.md with name and description."));
			await cleanup(tempDir);
			process.exit(1);
		}
		if (!blobResult) spinner$2.stop(`Found ${import_picocolors.default.green(skills.length)} skill${skills.length > 1 ? "s" : ""}`);
		if (options.list) {
			console.log();
			log.step(import_picocolors.default.bold("Available Skills"));
			const groupedSkills = {};
		...
		if (options.skill?.includes("*")) {
			...
			selectedSkills = selected;
			skillsToDisable = getDeselectedEnabledSkillNames(enabledSkills, selectedSkills, (skill) => skill.name);
		}
		if (selectedSkills.length === 0) {
			if (skillsToDisable.length === 0) {
				outro(import_picocolors.default.dim("No changes."));
		...
		spinner$2.start("Installing skills…");
		const results = [];
		...
		spinner$2.stop("Installation complete");
		console.log();
		const successful = results.filter((r) => r.success);
		const failed = results.filter((r) => !r.success);
		if (failed.length === 0) await applySkillRemovals(skillsToDisable, installGlobally);
		...
		if (successful.length > 0 && installGlobally && !directDownload) {
			const successfulSkillNames = new Set(successful.map((r) => r.skill));
			let cachedTree;
			if (parsed.type === "github" && normalizedSource && !blobResult) cachedTree = await fetchRepoTree(normalizedSource, parsed.ref, getGitHubToken);
			for (const skill of selectedSkills) {
...
async function runSync(args, options = {}) {
	...
	spinner$1.start("Scanning node_modules for skills…");
	const discoveredSkills = await discoverNodeModuleSkills(cwd);
	if (discoveredSkills.length === 0) {
		spinner$1.stop(import_picocolors.default.yellow("No skills found"));
		outro(import_picocolors.default.dim("No SKILL.md files found in node_modules."));
		return;
	}
	spinner$1.stop(`Found ${import_picocolors.default.green(String(discoveredSkills.length))} skill${discoveredSkills.length > 1 ? "s" : ""} in node_modules`);
	...
	if (options.agent?.includes("*")) {
		...
		spinner$1.start("Loading agents…");
		const installedAgents = await detectInstalledAgents();
		const totalAgents = Object.keys(agents).length;
		spinner$1.stop(`${totalAgents} agents`);
		if (installedAgents.length === 0) if (options.yes) {
	...
	spinner$1.start("Syncing skills…");
	const results = [];
